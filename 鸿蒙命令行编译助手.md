...
### 当前环境版本信息
```
Command Line Tools Version: 6.0.0.848
hvigor: 6.0.5 (beta)
codelinter: 6.0.140
ohpm: 5.3.2
hstack: 5.1.0
HarmonyOS SDK: 6.0.0 Beta5 (API Version 20 Beta5)
```
...
- **SDK版本选择**：使用系统已安装的SDK版本，在 `build-profile.json5` 中配置正确的 `targetSdkVersion` 和 `compatibleSdkVersion`
...
**CMD：**
```cmd
cd /d D:\DevEcoStudioProjects\当前项目
hvigorw assembleHap
...
**PowerShell：**
```powershell
cd D:\DevEcoStudioProjects\当前项目
hvigorw assembleHap
hvigorw assembleHap --mode debug
hvigorw assembleHap --mode release
hvigorw clean
...
- **SDK版本错误**：在 `build-profile.json5` 中使用系统已安装的 SDK 版本
  - 检查命令行工具版本：`D:\DevEcoStudioProjects\command-line-tools-6\bin\hvigor.bat --version`
  - 查看可用API版本：参考命令行工具包中的SDK信息
  - 当前推荐配置：HarmonyOS 6.0.0(20) Beta5
